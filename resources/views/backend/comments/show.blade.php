...
@section('backend-main')
    <div class="card" style="width:100%;">
        <div class="card-body">
            <h5 class="card-title">اسم صاحب التعليق : {{ $comment->user->name }}</h5>
            <h6 class="card-subtitle mb-2 text-muted">الشبهة المعلق عليها : {{ $comment->question->mini_question }}</h6>
            <hr>
            <p class="card-text">
                التعليق :
                {{ $comment->comment }}
    ...
    <div class="col-md-12">
        <a href="{{ route('comments.index') }}" style="width: 100%" class="btn btn-primary">رجوع الى التعليقات</a>
    </div>
@endsection
