            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">تعديل المشرف {{ $moderator->name }}</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <form method="post" action="{{ route('moderators.update' , $moderator->id) }}" id="myform">
                                @csrf
                                {{ method_field('PATCH') }}
                                <div class="form-row">
                                    <div class="col-md-12 col-12 mb-3">
                                        <label class="mb-1" for="name">الاسم</label>
                                        <input type="text" name="name" class="form-control" id="name" placeholder="الاسم" value="{{ $moderator->name }}" required>
                                        @error('name')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-md-12 col-12 mb-3">
                                        <label class="mb-1" for="email">البريد</label>
                                        <input type="email" name="email" class="form-control" id="email" placeholder="البريد" value="{{ $moderator->email }}" required>
                                        @error('email')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-md-12 col-12 mb-3">
                                        ...
                                        <input type="password" name="password" class="form-control" id="password" placeholder="الرقم السري" >
                                        @error('password')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-md-12 col-12 mb-3">
                                        <label class="mb-1" for="email">اعادة الرقم السري</label>
                                        <input type="password" name="confirm-password" class="form-control" id="اعادة الرقم السري" placeholder="confirm password" >
                                    </div>
                                    <div class="col-md-12 col-12 mb-3">
                                        <label>
                                            الصلاحيات
                                        ...
                                        <li class="d-inline-block mr-5">
                                            <fieldset>
                                                <div class="vs-checkbox-con vs-checkbox-primary">
                                                    <input @if($moderator->status == 1) checked @endif name="status" type="checkbox" value="1">
