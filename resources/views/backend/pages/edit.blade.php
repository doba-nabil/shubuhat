            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">تعديل الصفحة {{ $page->title_ar }}</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <form method="post" action="{{ route('pages.update' , $page->id) }}" enctype="multipart/form-data" id="myform">
                                @csrf
                                {{ method_field('PATCH') }}
                                <div class="form-row">
                                    <div class="col-md-12 col-12 mb-3">
                                        <label for="title">الاسم</label>
                                        <input type="text" name="title" class="form-control" id="title" placeholder="الاسم بالعربية" value="{{ $page->title }}" required>
                                        @error('title')
                                    ...
                                    <div class="col-md-12 col-12 mb-3">
                                        ...
                                        <textarea rows="10" name="text" class="form-control summernote" placeholder="المحتوى" required>{{ $page->text }}</textarea>
                                        @error('text')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-lg-12 col-md-12 col-image">
                                        <fieldset class="form-group">
                                            <label for="basicInputFile">الصورة الرئيسية </label>
                                        ...
                                        <div class="text-center">
                                            <img id="blah" src="{{ asset('pictures/pages/' . $page->mainImage->image) }}" alt="{{ $page->title }}" />
                                        </div>
                                        <br>
                                    </div>
