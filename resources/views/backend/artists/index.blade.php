                        ...
                        <div class="btn-dropdown mr-1 mb-1">
                            <div class="btn-group dropdown actions-dropodown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a class="dropdown-item" href="{{ route('artists.create') }}"><i class="fa fa-plus"></i>اضافة جديد</a>
                                    <a class="dropdown-item delete-all" onclick="return false;" delete_url="/delete_artists/">
                                        حذف الكل</a>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table data-thumb-view">
                            ...
                            <tr class="delete-all-cats">
                                <td></td>
                                <td class="product-img"><img src="{{ asset('pictures/artists/' . $artist->image) }}" alt="{{ $artist->name_en }}">
                                </td>
                                <td class="product-name">{{ $artist->name_ar }} / {{ $artist->name_en }}</td>
                                <td class="product-category">{{ $artist->category->title_ar ?? '' }} / {{ $event->category->title_en ?? '' }} </td>
                                <td>
                                    {{ $artist->getActive() }}
                                </td>
                                <td class="product-action">
                                    <span class="action-edit"><a href="{{ route('artists.edit' , $artist->slug) }}"><i class="feather icon-edit"></i></a></span>
                                    <a title="" onclick="return false;" object_id="{{ $artist->id }}"
                                       delete_url="/artists/" class="edit-btn-table remove-alert" href="#">
