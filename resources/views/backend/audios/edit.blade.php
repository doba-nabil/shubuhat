            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">تعديل ملف الصوت {{ $audio->title }}</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <form method="post" action="{{ route('audios.update' , $audio->id) }}" enctype="multipart/form-data" id="myform">
                                @csrf
                                {{ method_field('PATCH') }}
                                <div class="form-row">
                                    <div class="col-md-12 col-12 mb-3">
                                        <label for="title">العنوان</label>
                                        <input type="text" name="title" class="form-control" id="title" placeholder="الاسم بالعربية" value="{{ $audio->title }}" required>
                                        @error('title')
                                    ...
                                    <div class="col-md-12 col-12 mb-3">
                                        ...
                                        <input name="body" class="form-control" placeholder="عنوان فرعي" value="{{ $audio->body }}" required>
                                        @error('body')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                        ...
                                        <div class="form-group">
                                            <select name="category_id" class="select2 form-control">
                                                @foreach($categories as $parent)
                                                    <option
                                                            @if($audio->category_id == $parent->id) selected @endif
                                    ...
                                    <hr>
                                    <div class="col-md-12 col-12 mb-3">
                                        <input @if($audio->kind == 0) checked @endif type="radio" id="linkRadio" name="kind" value="0">
                                        <label for="linkRadio">رابط</label><br>
                                        <input @if($audio->kind == 1) checked @endif type="radio" id="fileRadio" name="kind" value="1">
                                    ...
                                    <div class="col-md-12 col-12 mb-3 link">
                                        <label for="link">رابط الــ SoundCloud</label>
                                        <input type="text" name="link" class="form-control" id="link" placeholder="رابط الــ SoundCloud" value="{{ $audio->link }}">
                                        <br>
