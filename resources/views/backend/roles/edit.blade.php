            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">تعديل المجموعة {{ $role->name }}</h4>
                    </div>
                            ...
                            <form method="post" action="{{ route('roles.update' , $role->id) }}" id="myform">
                                ...
                                <div class="form-row">
                                    <div class="col-md-12 col-12 mb-3">
                                        ...
                                        <input type="text" name="name" class="form-control" id="name" placeholder="اسم المجموعة ( احرف انجليزية بدون مسافات )" value="{{ $role->name }}" required>
                                        @error('name')
                                            ...
                                            <select style="width: 100%" name="permission[]" class="select2 form-control select2-hidden-accessible" multiple="" data-select2-id="7" tabindex="-1" aria-hidden="true">
                                                @foreach($permission  as $value)
                                                    <option
                                                        ...
                                                        @endforeach
                                                       value="{{ $value->id }}">{{ $value->name_ar }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
